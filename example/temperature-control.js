...
function processTest(args) {
    ...
    commands: {
      mode: {
        callback: function () {
          ...
          var enabledStatus =
            deviceControlState.enabled === true ? "OFF" : " ON";
          deviceControlState.enabled =
            deviceControlState.enabled === true ? false : true;
          ...
          if (renderScreen === true) {
            log.log(
              "temperature control " +
                (deviceControlState.enabled ? "enabled" : "disabled")
            );
  ...
  thingShadows.on("status", function (
    thingName,
    statusType,
    clientToken,
    stateObject
  ...
  thingShadows.on("delta", function (thingName, stateObject) {
    if (networkEnabled === true) {
      if (thingName === "TemperatureControl") {
        if (
          !isUndefined(stateObject.state.enabled) &&
          stateObject.state.enabled !== deviceControlState.enabled
        ) {
          log.log(
            "temperature control " +
              (stateObject.state.enabled ? "enabled" : "disabled")
          );
        }
...
if (require.main === module) {
  cmdLineProcess(
    "connect to the AWS IoT service and demonstrate thing shadow APIs, test modes 1-2",
